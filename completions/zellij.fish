...
complete -c zellij -n "__fish_seen_subcommand_from attach; and not __fish_seen_subcommand_from help options" -s "c" -l "create" -d "Create a session if one does not exist"
complete -c zellij -n "__fish_seen_subcommand_from attach; and not __fish_seen_subcommand_from help options" -s "h" -l "help" -d "Print help information"
complete -c zellij -n "__fish_seen_subcommand_from attach; and not __fish_seen_subcommand_from help options" -l "index" -d "Number of the session index in the active sessions ordered creation date" -x

complete -k -c zellij -n "__fish_seen_subcommand_from attach; and not __fish_seen_subcommand_from help options" -x -a options -d "Change the behaviour of zellij"
complete -k -c zellij -n "__fish_seen_subcommand_from attach; and not __fish_seen_subcommand_from help options" -x -a help -d "Print this message or the help of the given subcommand(s)"


complete -c zellij -n "__fish_seen_subcommand_from attach; and __fish_seen_subcommand_from options" -l "attach-to-session" -d "Whether to attach to a session specified in \"session-name\" if it exists [possible values: true, false]" -x
complete -c zellij -n "__fish_seen_subcommand_from attach; and __fish_seen_subcommand_from options" -l "copy-clipboard" -d "OSC52 destination clipboard [possible values: system, primary]" -x
complete -c zellij -n "__fish_seen_subcommand_from attach; and __fish_seen_subcommand_from options" -l "copy-command" -d "Switch to using a user supplied command for clipboard instead of OSC52" -x
complete -c zellij -n "__fish_seen_subcommand_from attach; and __fish_seen_subcommand_from options" -l "copy-on-select" -d "Automatically copy when selecting text (true or false) [possible values: true, false]" -x
complete -c zellij -n "__fish_seen_subcommand_from attach; and __fish_seen_subcommand_from options" -l "default-layout" -d "Set the default layout" -x
complete -c zellij -n "__fish_seen_subcommand_from attach; and __fish_seen_subcommand_from options" -l "default-mode" -d "Set the default mode" -x
complete -c zellij -n "__fish_seen_subcommand_from attach; and __fish_seen_subcommand_from options" -l "default-shell" -d "Set the default shell" -x
...
complete -c zellij -n "__fish_seen_subcommand_from attach; and __fish_seen_subcommand_from options" -l "mirror-session" -d "Mirror session when multiple users are connected (true or false) [possible values: true, false]" -x
complete -c zellij -n "__fish_seen_subcommand_from attach; and __fish_seen_subcommand_from options" -l "mouse-mode" -d "Set the handling of mouse events (true or false) Can be temporarily bypassed by the [SHIFT] key [possible values: true, false]" -x
...
complete -c zellij -n "__fish_seen_subcommand_from attach; and __fish_seen_subcommand_from options" -l "on-force-close" -d "Set behaviour on force close (quit or detach)" -x
complete -c zellij -n "__fish_seen_subcommand_from attach; and __fish_seen_subcommand_from options" -l "pane-frames" -d "Set display of the pane frames (true or false) [possible values: true, false]" -x
...
complete -c zellij -n "__fish_seen_subcommand_from attach; and __fish_seen_subcommand_from options" -l "session-name" -d "The name of the session to create when starting Zellij" -x
complete -c zellij -n "__fish_seen_subcommand_from attach; and __fish_seen_subcommand_from options" -l "simplified-ui" -d "Allow plugins to use a more simplified layout that is compatible with more fonts (true or false) [possible values: true, false]" -x
complete -c zellij -n "__fish_seen_subcommand_from attach; and __fish_seen_subcommand_from options" -l "theme" -d "Set the default theme" -x
complete -c zellij -n "__fish_seen_subcommand_from attach; and __fish_seen_subcommand_from options" -l "theme-dir" -d "Set the theme_dir, defaults to subdirectory of config dir" -r
